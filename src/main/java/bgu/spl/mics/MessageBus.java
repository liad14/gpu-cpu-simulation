...
public interface MessageBus {
     ...
     * @post for(Microservice m:getSubscribers(b))isSubscribed(b,m)
     */
     ...
     * @pre !hasBeenSent(e)
     * @post if(getSubscribers(e).isEmpty()) return null
     * @post hasBeenSent(e)
     * @post !isAvailable(getMicro(e))
     * @post for(MicroService m:getMicroServices())if(m != getMicro(e) getEvent(m) != e
     */
     ...
     * @post !isRegistered(m)
     * @post for(Class<? extends Message> message:getMessagesTypes) !isSubscribed(message,m)
     * @post getEvent(m)==null
     */
     ...
     *                              to became available.
     * @pre isRegistered(m)
     * @post if(getMessages(m).isEmpty()) isAvailable(m)
     * @post if(!getMessages(m).isEmpty())!isAvailable(m)
     */
    ...
    //List<MicroService> getSubscribers(Class<? extends Message> type);
    <T extends Message>boolean hasBeenSent(Message type);
    boolean isRegistered(MicroService m);
    boolean isAvailable(MicroService m);
    //List<MicroService> getRegisteredMicros();
    <T> MicroService getMicro(Event<T> e);
    List<Class<? extends Message>> getMessagesTypes();
    <T> Event<T> getEvent(MicroService m);
